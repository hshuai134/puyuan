...
const FirstGloor = ({ flag, errData = [], path = [] }) => {
  ...
  return (
    <div className='girst_floor'>
      ...
      {showErrPath.map((item, i) => {
        return <Path item={item} flag={flag} x={40} y={-510} key={i} isErr={true} />
      })}
      {myPath.map((item, i) => {
        return <Path item={item} flag={flag} x={40} y={-510} key={item.num} isErr={false} />
      })}
    </div>
  )
}

export default FirstGloor