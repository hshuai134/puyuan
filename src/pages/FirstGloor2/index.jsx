...
const FirstGloor2 = ({ flag, errData = [], path = [] }) => {
  ...
  return (
    <div className='girst_floor2'>
      ...
      {showErrPath.map((item, i) => {
        return <Path item={item} flag={flag} x={2040} y={-71} key={i} isErr={true} />
      })}
      {myPath.map((item, i) => {
        return <Path item={item} flag={flag} x={2040} y={-71} key={item.num} isErr={false} />
      })}
    </div>
  )
}

export default FirstGloor2