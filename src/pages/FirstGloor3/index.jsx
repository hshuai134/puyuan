...
import './index.less'
const FirstGloor3 = ({ flag, errData = [], path = [] }) => {
  ...
  return (
    <div className='girst_floor3'>
      ...
      {/* errPosition showErrPath */}
      {showErrPath.map((item, i) => {
        return <Path item={item} flag={flag} x={140} y={1520} key={i} isErr={true} />
      })}
      {myPath.map((item, i) => {
        return <Path item={item} flag={flag} x={140} y={1520} key={item.num} isErr={false} />
      })}
    </div>
  )
}

export default FirstGloor3