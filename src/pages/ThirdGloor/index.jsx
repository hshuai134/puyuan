...
const ThirdGloor = ({ flag, errData = [], path = [] }) => {
  ...
  return (
    <div className='third_gloor'>
      ...
      {showErrPath.map((item, i) => {
        return <Path item={item} flag={flag} x={3540} y={1340} key={i} isErr={true} />
      })}
      {myPath.map((item, i) => {
        return <Path item={item} flag={flag} x={3540} y={1340} key={item.num} isErr={false} />
      })}
    </div>
  )
}

export default ThirdGloor